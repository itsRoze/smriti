...
### Added
- **A description can hold a photo.** Paste a screenshot into any description on the board — an app's, a project's, or a ticket's body, which are one editor wearing three hats — and it is stored and rendered in place; dropping an image file does the same. The text gains `![caption](smriti://photo/12)`, a self-describing reference rather than a URL, so it still means something in `smriti ticket show` or to an agent. The bytes go in `factory.db` (a new `photos` table, so no migration and no version bump — `factory-schema.sql` is re-applied on every connection and is all `CREATE ... IF NOT EXISTS`), for the same reason `documents.body` does: a screenshot on the desktop is gone in a month and the ticket that needed it never says so. Inlining the image into the text was measured and rejected — every description is re-read about once a second to keep the board live, so one 3 MB screenshot would ride along in every tick forever.
- **`smriti photo add | list | show | data | rm | prune`.** `add` reads **stdin** when no file is named, which is the whole reason the board never had to invent a spill directory: the CLI owns the temp file start to finish, because `sqlite3` loads a blob with `readfile()` and there is no way to hand it bytes on a pipe. `data` answers with the mime on one line and the bytes as hex on the next — hex because a shell cannot carry raw binary out of a subprocess, and `writefile('/dev/stdout', …)` was tested and appends its own return value to the stream, corrupting the image.
- **The format is read from the bytes, never from what the caller claimed.** PNG, JPEG, GIF and WebP by signature; everything else refused with exit 5. An SVG is refused however it is labelled and whatever it is named — it is a program, and this renders inside the board. The 10 MB ceiling (`SMRITI_PHOTO_MAX_BYTES`) is enforced by the board *while the upload is arriving*, not after the whole of it has been buffered and handed to a subprocess.
- **Photo ids are never reused** (`AUTOINCREMENT`, the one place in this schema that needs it). The board caches a photo permanently by id, which is only sound because a given id is one immutable picture for the life of the store — and a plain `INTEGER PRIMARY KEY` hands a deleted row's number to the next insert, so deleting #12 and pasting a new photo would have shown the new picture everywhere the old one still appeared, straight out of the browser cache. Confirmed by test before it was written, and locked by one after.
- **Deleting a photo's line from a description deletes the photo.** The sweep lives in the write verbs rather than in the board, so it holds for a write made from the command line or by an agent as much as from a paste: `ticket edit --body`, `project edit --description`, `repo edit --description`, and equally the verbs that remove text wholesale rather than editing it — `ticket rm`, `project rm`, `repo forget`, `ticket doc-forget`. Only the ids that write dropped are considered, and each is still checked against every other description, body and stored document before deletion, so removing one of two copies of the same screenshot leaves the picture alone. It is best-effort by design: the text is already saved by the time it runs, and failing a save the user watched succeed would be a worse bug than bytes that `prune` collects later.
- **`smriti photo prune` is the broom for what a write never saw.** Paste and then press Escape and nothing was saved, so nothing swept — those bytes sit unreferenced. `prune` reads all four stored surfaces **and** the document working copies under `~/.smriti/projects/<slug>/`, because `ticket doc` stores no body for a path outside that tree and `doc-show` syncs from the file on every read — so those references are genuinely on screen. `--ids` narrows *which* photos are considered without ever weakening the question asked about them, which is what makes it safe to run on every write. Ids are compared with one spelling per number, so `smriti://photo/007` and `smriti://photo/7` are the same picture to the sweep as they already were to the renderer and to SQL.
- **The paste survives being slower than the editor around it.** The description editor is rebuilt about once a second by the SSE redraw, saves on blur and abandons on Escape — all of which can happen mid-upload. So a placeholder goes in at once and is swapped for the real reference by exact string match (surviving whatever else was typed meanwhile), the redraw is held off while anything is in flight, and a save **waits for its own editor's uploads** rather than writing `uploading…` into the description and losing the picture to a body that says so forever. An upload that lands after its editor closed is dropped rather than written into whatever is on screen now — and its photo is exactly what `prune` exists to collect.
- **The factory holds the plan, not a path to it.** `documents` was an index into files on disk, and `smriti clean` deleted those files — then `doc-forget` deleted the rows — the moment a branch merged. The record of how a change was made died at exactly the moment the change succeeded: ship a ticket and there was no trace at all, not a dangling pointer, no row. Measured on this machine before the change: 4 rows against 40 markdown files. The body now lives in `factory.db` (`documents.body` + `captured_at`, schema v6 — two columns in one transaction, because a half-applied pair would be stamped current and `factory-schema.sql` can only `CREATE`). The file becomes a **working copy**: it wins while it exists, the store wins once it is gone, and the two can never disagree because every read goes through `smriti ticket doc-show`, which syncs from disk first. Content is read by sqlite3's `readfile()` — `$(cat f)` eats the trailing newline and a 2 MB literal is over `ARG_MAX` — and the write is verified byte-for-byte rather than assumed.
- **`smriti clean` deletes only what it has stored.** Per file, not per branch. It captures the branch's documents first, and anything it could not store — unreadable, over the 2 MB ceiling, a database that would not take it — is left exactly where it is with a line saying so, while the clean itself still exits 0 and finishes the sweep. Making a capture failure *fail* the clean was considered and rejected: `purge_branch_artifacts` is deliberately best-effort and `--all` deliberately skip-and-continue, and a bookkeeping failure aborting a clean would be a worse bug than the one being fixed. The capture pass also **adopts what it finds** — the purge globs filenames and never consulted the index, so a file written but never registered used to be deleted with nothing anywhere recording that it had existed.
- **`smriti ticket doc-show <id> [--json]`, `doc-capture --branch B`, `doc-adopt [--dry-run]`.** `doc-show` is the one read path for a body, and reports "never captured" as its own answer rather than an error — a plan registered a moment before it is written is in exactly that state every run. `doc-adopt` is the one-shot that took in this machine's 29 orphans (581 KB of markdown; `factory.db` 192 KB → 856 KB), parsing slug/branch/type/date out of the existing filename contract and reporting the 7 pre-v1.0 filenames it declines to guess at.
- **Branch matching tolerates both forms of a name.** `/` becomes `--` in a filename and that is not reversible, so a row adopted from disk may hold either. Every branch-scoped query matches the raw name *or* its slug form — one extra `OR`, and the ambiguity stops mattering, where guessing would silently drop an adopted row out of every later operation.

- **A ticket can say what has to land before it.** The backlog already carried its own sequencing — *"BLOCKED on #7 items 3 and 6"*, *"Depends on #12 and #32"*, *"do not start this before them"* — written in prose, where nothing could read it. `smriti ticket dep <id> --blocks <id>` and `--blocked-by <id>` record it instead, in a `ticket_deps` edge table: many-to-many in both directions, because a ticket can block several and be blocked by several, and `ON DELETE CASCADE` at both ends because an edge with one end missing is not a weaker edge, it is not an edge. A new table needs no migration and no version bump, the same property `run_artifacts` relied on. Nothing is stored on the tickets themselves — blocked, freed and startable are derived on every read — so un-cancelling a blocker re-blocks its dependents with nothing to reconcile, which is what keeps `cancel`'s advertised reversibility true.
- **`smriti ticket start` refuses work that cannot land.** It lists the blockers still open and exits 7, with `--force` when you disagree; the board turns that into a 409 and offers the second press rather than reporting that the factory fell over. The ticket asked for "refuse, or warn loudly" — refusal is the half a machine can act on, and the autonomous dispatcher this is sequenced ahead of cannot be asked to parse a warning off stderr. Re-attaching an in-progress worktree is never refused: the work is already underway, so `cd "$(smriti ticket start N)"` mid-run and the board's restart both still work.
- **`smriti ticket list --unblocked` — the frontier, and it means *startable*.** Every blocker shipped or cancelled, the ticket itself still open, **and** not already started. That last clause is the load-bearing one: `start` is idempotent, so a frontier meaning only "blockers satisfied" would hand a dispatcher work that is already running, forever, and `start` would keep saying yes. `--blocked` is its complement, and an already-started ticket appears in neither. A cycle would make the question unanswerable, so one is refused on write — self-links too — with the check riding on the insert itself so it and the write share a lock.
- **Shipping tells you what it freed.** `smriti ticket done`, `cancel`, and the `smriti clean` at the tail of `/begin` name what just became startable. This is the moment the graph pays for itself: you finish something and learn what it unlocked, instead of finding out a week later.
- **The board draws it, quietly.** A blocked card is faded and dashed with what it waits on named on it — deliberately not orange and not highlighter, which mean *live* and *you are the hold-up*; blocked is neither, it is *not yet*. A ticket page gains **blocked by** and **blocks** rows that link through and name the far end's app when the edge crosses one, and `w` ("waits on") opens a picker to draw or cut an edge. When the last blocker lands, the dependents read **freed** and are announced under *waiting on you* — as a subordinate group below the real gates, in pine rather than highlighter, clearing when you act on one rather than on a timer. Edges may cross projects and apps, because the API ticket blocking the UI ticket is the ordinary case.
- **A manual order that contradicts the graph is surfaced, never corrected.** Drag a blocked ticket above its blocker and the board says so and leaves it exactly where you put it. You may be sequencing deliberately — about to `--force`, or about to cancel the blocker — and a board that rearranges your work while you are arranging it is the worse tool.
- **Drag a ticket into the order you mean to work it, and it stays there.** The board could not express the one thing you most want to tell it — *do these in this order* — because a card's place was computed and never chosen. Cards now carry a `position` (schema v5), and dragging one writes the midpoint of its two neighbours: one row per drag, no renumbering cascade, and the group repacks to whole numbers only when a gap has been subdivided into the ground. `⇧J` / `⇧K` does the same without the trackpad — `esc` puts the card back, `⏎` drops it — because a mouse-only reorder would be the one action on a keyboard-driven board you had to reach for the trackpad to perform. An order is scoped to the group a card is drawn in: a project's tickets, an app's loose tickets, or the app-less ideas. You cannot drag a card into another group, because that is re-filing and `f` already does it.
- **Reordering, in the board's own hand.** Generic drag interfaces fade the thing you are dragging and draw a line where it lands; on a board drawn as paper that is backwards. The card in your hand stays the most *solid* thing on the page — tilted past hover, scaled up, its hard-edged shadow thrown three times further, and leaning into the throw as you fling it — while the absence it left is the ghost. Where it lands is a card-shaped highlighter **slot**, not an insertion line: these cards sit in a responsive grid where the drop point can fall mid-row, so the slot takes a real cell and the browser's own auto-placement works out the rest, with the siblings gliding there on the same spring the cards already enter on. Nothing permanent was added to a card — no grip handle, no drag dots. The affordance is the cursor plus the lift hover was already doing.
- **`smriti ticket move <id> --before|--after <id>|--top|--bottom`.** The board reaches it over HTTP and never computes a position itself, so the drag and the keyboard land on one place that owns the arithmetic. Naming a target in another app or project is refused (exit 6, and a 409 rather than a 500 on the route) — an illegal drop is your mistake, not the factory falling over, and the page can tell them apart.
- **A run now records what it concluded, not just how long it took.** `run_artifacts` (a new table, so no migration — `factory-schema.sql` is re-applied on every connection and is all `CREATE ... IF NOT EXISTS`) stores the closing summary `/begin` writes at Gate 3, via `smriti trace report`. Until this existed that text lived nowhere but the pixels of a herdr pane, so closing the pane destroyed the most valuable thing the run produced. The table is general from the outset — `kind`, `source`, `status`, `body` or `path` — because the evidence record (tests green, audit clean, screenshots) is the same substrate from the other side and had to be designed once rather than twice. The board shows the report at the top of a finished run's body, above the timing bar: what it concluded before how long it took.
- **The board closes a session once its ticket has shipped.** A sweep every ten seconds (plus one at startup) looks for tickets that are shipped, whose worktree is gone, and whose herdr agent has finished — and closes the pane. Not `smriti clean`, which almost always runs *inside* the session being closed, where closing the pane kills the process running the command and loses its exit status. Opt out with `close_session_on_ship=false`; capture keeps running either way.
- **Nothing is closed that has not first been captured.** A stored report is the licence to close. A run that left none — it died before Gate 3, or the session was never `/begin` — gets its pane scraped first and the record marked `source=pane`, shown on the board as *"recovered from the terminal — may be partial"* rather than passed off as the run's own words. A pane that cannot be captured is left open, because losing the only copy of what a run said is the bug this exists to prevent.
- **`runs.herdr_pane`** (schema v4, a metadata-only `ALTER TABLE ADD COLUMN` like v3). `trace start` reads `$HERDR_PANE_ID`, which herdr injects into every pane it manages, so a run knows its own pane exactly. That is a handle which survives `smriti clean` deleting the worktree — previously the only thing tying a session to a ticket — and it keeps the lookup off herdr's agent *names*, which are machine-global (`t<id>`) and had already let a test fixture collide with a live session.
...
### Changed
- **`/api/doc/:id` stops reading files.** It was the board's one exception to "read nothing outside the store" — a realpath-confined, size-capped read of a path fetched by a client-supplied id. It asks `smriti ticket doc-show --json` now, so the *server* touches no path at all. The confinement did not disappear with it, though, and saying so would have been the bug: the read moved into `capture_doc`, and the check moved with it, because unconfined it would turn any bad `documents.path` into a file-read-and-*persist* oracle rather than a 403. A working copy must be an absolute path, under `$SMRITI_HOME/projects`, and not a symlink. The board's remaining file read (`PROJECT.md`/`DESIGN.md`, off a fixed two-name allowlist under a root the CLI supplies) is untouched. The client also **refetches on click** — it cached a rendered document by id forever, so opening a plan you had revised in between showed the stale render, which is the one failure a document viewer must not have because it looks correct. The cache goes back to being what it was for: surviving the board's once-a-second redraw.
- **A relative `--path` is absolutized at registration.** It used to be stored verbatim, which was merely an unhelpful index entry. Now every read syncs *from* that path, so a relative one resolves against whatever directory the reader is standing in — and the board's reader is a server process standing wherever it was launched. One read from the wrong cwd would have overwritten a stored plan with an unrelated file of the same name.
- **`smriti ticket rm` destroys the documents' content, not just pointers to it**, and both confirmations now say so — the CLI's, and the board's, which had been a generic "really delete?" written for the old index-only semantics and now reads *"delete + 3 docs?"*. Deliberate: `rm` is explicit, already counts what it will take, and already asks — a delete verb that secretly keeps things is its own trap. A working copy still on disk is untouched.
- **One order, and the board is the surface that defines it.** `smriti ticket list` sorted by `priority DESC, id ASC`; the board sorted by status band, then priority, then id; `smriti factory` had a third opinion of its own. Every surface now reads `position`. The migration seeds it from what the board was already drawing, so **the board is unchanged until you drag something** — it is the CLI listing that moves, adopting the board's order, which is exactly the split closing. Consequence worth knowing: **`--priority` no longer changes where a ticket appears.** It stays a field you can set and read, but sequence belongs to `position` now; today's priority-driven order is baked into the seeded positions rather than lost. Completed work is deliberately exempt — the history fold keeps its status sort and is not draggable, because a hand-placed sequence over shipped tickets is a number nobody reads.
...
### Fixed
- **"Nothing is closed that has not first been captured" held in one of the two places the board closes a pane.** The other is the restart button, which closed the old pane having read nothing — so restarting destroyed everything the session had printed, on the one action you reach for *when a run has gone wrong* and its output is therefore worth most. Capture-then-close is now a single helper both call sites go through, so the invariant sits where panes are closed rather than being remembered per caller. The two deliberately disagree about exactly one thing, and it is named in the code rather than implied: an uncapturable pane is left **open** by the ship sweep, which is unattended housekeeping nobody asked for, and **closed anyway** by restart, because you asked for that session to be replaced and refusing would leave you holding the broken one. The window is narrower than it sounds — a pane already gone fails the read and no-ops the close. A replaced run is also ended now (`failed`, because it did not finish), instead of ticking a live duration forever for a session that no longer exists. The ticket predicted this could not share the sweep's helper, on the grounds that a working agent's pane cannot be read; it can — `pane read` answers the same whatever the lifecycle state, on all four `--source` modes, and `agent_not_idle` belongs to herdr's *attach* path.
- **Part of the restart route came off the board's event loop**, on a file whose own comments twice explain why blocking it must not happen. The pane close, the deliberate `Bun.sleepSync(500)`, `smriti ticket start` and the agent lookup are all async now; during any of them the board used to stop answering every other request, including the poll that redraws it. `launchSession` is still synchronous and is the bigger offender — up to eight `agent start` attempts a second apart — but it is shared with `/api/tickets/:id/start` and untangling it is its own change, so this is an improvement rather than a fix. Restart is also guarded now: a second restart, or a `start` arriving in the window where the old pane is closed and its replacement is not yet up, is refused with a 409 instead of launching a duplicate. That race was narrow while the route was synchronous end to end, and capturing first makes it slower and reachable.
- **A herdr agent named `t<id>` in an unrelated directory could be mistaken for a ticket's session — and then closed.** Agent names are machine-global, which is how a test fixture came to adopt a live session during ticket 3. `existingAgent()` matched name *or* worktree, so the fix is not a reordering: every caller passes the worktree, which means a name-only hit is by definition a session somewhere else. It matches on `cwd` alone now. The old predicate was also one `.find()` with an OR rather than the preference its comment claimed — "matched on the worktree path first" was never true, and whichever session herdr happened to list first won. No test had ever covered the clause, because no herdr stub in the suite emitted a `name` field at all; one does now.
