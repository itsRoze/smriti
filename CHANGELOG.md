...
### Changed
- **`/api/doc/:id` stops reading files, and the traversal risk class goes with it.** It was the board's one exception to "read nothing outside the store" — a realpath-confined, size-capped read of a path fetched by a client-supplied id. It asks `smriti ticket doc-show --json` now, so the server touches no path at all and the guard is gone because the thing it guarded is gone. The board's remaining file read (`PROJECT.md`/`DESIGN.md`, off a fixed two-name allowlist under a root the CLI supplies) is untouched. The client also **refetches on click** — it cached a rendered document by id forever, so opening a plan you had revised in between showed the stale render, which is the one failure a document viewer must not have because it looks correct. The cache goes back to being what it was for: surviving the board's once-a-second redraw.
- **`smriti ticket rm` destroys the documents' content, not just pointers to it**, and the confirmation now says so. Deliberate: `rm` is explicit, already counts what it will take, and already asks — a delete verb that secretly keeps things is its own trap. A working copy still on disk is untouched.
