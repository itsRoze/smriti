...
- **`/begin` investigates root cause when the request is a bug.** A symptom, a stack trace or an issue reference routes through reproduce → trace backward → gapless causal chain before anything gets planned, and writes the same `<branch>-debug-<ts>.md` artifact as before. A plan written over an unknown cause is a guess.
