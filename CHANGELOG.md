...
- **Deleting a photo's line from a description deletes the photo.** The sweep lives in the three write verbs — `ticket edit --body`, `project edit --description`, `repo edit --description` — rather than in the board, so it holds for a write made from the command line or by an agent as much as from a paste. Only the ids that edit dropped are considered, and each is still checked against every other description, body and stored document before deletion, so removing one of two copies of the same screenshot leaves the picture alone. It is best-effort by design: the text is already saved by the time it runs, and failing a save the user watched succeed would be a worse bug than bytes that `prune` collects later.
- **`smriti photo prune` is the broom for what a save never saw.** Paste and then press Escape and nothing was saved, so nothing swept — those bytes sit unreferenced. `prune` reads every description, body and document, and deletes only what none of them mention; `--ids` narrows *which* photos are considered without ever weakening the question asked about them, which is what makes it safe to run on every write.
