...
- **The restart route blocked the board's whole event loop**, on a file whose own comments twice explain why that must not happen — a `spawnSync` pane close plus a deliberate `Bun.sleepSync(500)`, during which every other request, including the poll that redraws the board, simply stopped. Now async, and guarded so two restarts of one ticket cannot start two sessions against a single closed pane: that race was narrow while the route was synchronous, and capturing first makes it slower and reachable.
