...
**Removing a photo's line from a description deletes the photo.** That sweep lives in the write verbs rather than in the board, so it holds for a write made from the command line or by an agent too — `ticket edit --body`, `project edit --description` and `repo edit --description`, and equally the verbs that remove text wholesale: `ticket rm`, `project rm`, `repo forget` and `ticket doc-forget`. Only the ids that write dropped are considered, and each is still checked against every other description, body and document first — so removing one of two copies of the same screenshot leaves the picture alone. The sweep is best-effort: the text is already saved by the time it runs, and failing a write you watched succeed would be worse than bytes `prune` collects later.

`prune` is the broom for what a write never saw: paste, then press Escape, and nothing was saved, so nothing swept. It reads all four stored surfaces **and** the document working copies under `~/.smriti/projects/<slug>/` — a plan registered from elsewhere has a NULL body and a real file that `doc-show` syncs from on every read, so its references are genuinely on screen and must not be swept out from under it.
